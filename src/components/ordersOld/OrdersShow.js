...
const OrdersShow = (props) => {
	...
	for (const item of record.car) {
		if (item.cantidad < 6 || carLength < 5) {
							...
							<TableBody>
								{rows.map((row) => (
									<TableRow key={row.name}>
										...
										<TableCell align="right">
											{row.aviableColors.map((color) => '- ' + color)}
											<div
												style={{
													flexDirection: 'row',
													display: 'flex',
